...
def s_norm_graph_collate(batch: List[Tuple]):
    ...
    tab_snorm_n = [
        torch.FloatTensor(
            ...
            1).fill_(
            1. /
            float(size)) for size in tab_sizes_n]
...
def contrastive_vae_collate(batch: List[Tuple]):
    ...
    return [batched_graph], [batched_graph3d, torch.cat(
        pairwise_indices, dim=-1)], torch.cat(distances)
...
def pairwise_distance_collate(batch: List[Tuple]):
    ...
    mask = torch.arange(n_atoms.max(), device=distances[0].device)[
        None, :] >= n_atoms[:, None]  # [batch_size, n_atoms]
    return [batched_dgl_graph, torch.cat(
        pairwise_indices, dim=-1), mask], torch.cat(distances)
...
def contrastive_graphs_with_mask_collate(batch: List[Tuple]):
    ...
    # create mask corresponding to the zero padding used for the shorter sequences in the batch.
    # All values corresponding to padding are True and the rest is False.
    mask = torch.arange(n_atoms.max(), device=batched_dgl_graph.device)[
        None, :] >= n_atoms[:, None]  # [batch_size, n_atoms]
...
def contrastive_collate(batch: List[Tuple]):
    ...
    if targets:
        return [batched_graph], [
            batched_graph3d], torch.stack(*targets).float()
    else:
...
class NoisedDistancesCollate(object):
    ...
    def __call__(self, batch: List[Tuple]):
        ...
        for i in range(self.num_noised):
            ...
            copy_graph.edata['w'] += torch.randn_like(
                copy_graph.edata['w']) * self.std
...
class NoisedCoordinatesCollate(object):
    ...
    def __call__(self, batch: List[Tuple]):
        ...
        for i in range(self.num_noised):
            copy_graph = copy.deepcopy(batched_graph3d)
            copy_graph.ndata['x'] += torch.randn_like(
                copy_graph.ndata['x']) * self.std
            distances = torch.norm(
                copy_graph.ndata['x'][edges[0]] - copy_graph.ndata['x'][edges[1]], p=2, dim=-1)
...
class NodeDrop3dCollate(object):
    ...
    def __call__(self, batch: List[Tuple]):
        ...
        for graph3d in graphs3d:
            ...
            if remove_number > 0:
                remove_indices = torch.randint(
                    low=0, high=graph3d.number_of_nodes(), size=(
                        remove_number.data,), device=device)
...
class NodeDrop2dCollate(object):
    ...
    def __call__(self, batch: List[Tuple]):
        ...
        for graph in graphs:
            ...
            if remove_number > 0:
                remove_indices = torch.randint(
                    low=0, high=graph.number_of_nodes(), size=(
                        remove_number.data,), device=device)
                graph.remove_nodes(remove_indices)
        batched_graph = dgl.batch(graphs)
        batched_graph3d = dgl.batch(graphs3d)
...
def egnn_padded_collate3d(batch):
    ...
    # pad features with -1 because that is used as padding index in the atom
    # embedder in egnn
    ...
    return [batched_graph], [features, coordinates,
                             edges, None, atom_mask, edge_mask, n_nodes]
...
def egnn_padded_collate(batch):
    ...
    coordinates = coordinates.view(batch_size * n_nodes, -1)
    atom_mask = atom_mask.view(batch_size * n_nodes, -1).float()
    return [features, coordinates, edges, None, atom_mask,
            edge_mask, n_nodes], torch.stack(targets).float()
...
def pna_transformer_collate(batch):
    ...
    mask = torch.arange(features.shape[1])[
        None, :] >= n_atoms[:, None]  # [batch_size, n_atoms]
    return [dgl.batch(graphs), features, pos_enc,
            mask], torch.stack(targets).float()
...
def molhiv_padded_collate(batch: List[Tuple]):
    ...
    """
    graphs, targets = map(list, zip(*batch))
    features = pad_sequence([graph.ndata['feat']
                            for graph in graphs], batch_first=True)
...
def padded_distances_collate(batch):
    ...
    mask = torch.arange(padded.shape[1])[
        None, :] >= n_dist[:, None]  # [batch_size, n_atoms]
    batched_graph = dgl.batch(graphs)
