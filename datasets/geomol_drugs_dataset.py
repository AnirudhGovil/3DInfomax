...
class GeomolDrugsDataset(Dataset):
    ...
    def __init__(
        ...
        if 'conformations' in self.return_types or 'complete_graph_random_conformer' in self.return_types or 'pyg_multiple_conformers' in self.return_types:
            self.conformations = data_dict['coordinates'][:,
                                                          :self.num_conformers * 3]
            self.conformer_categorical = torch.distributions.Categorical(
                logits=torch.ones(self.num_conformers))
    ...
    def data_by_type(self, idx, return_type, e_start, e_end, start, n_atoms):
        ...
        if return_type == 'conformations':
            ...
            for i, slice in enumerate(slices[:-1]):
                neighbors.append(
                    self.meta_dict['neighbors_list'][slice: slices[i + 1]])
            return neighbors
            ...
            edge_features = self.e_features_tensor[e_start: e_end].to(
                self.device)
            edge_indices = self.edge_indices[:, e_start: e_end].to(self.device)
            pos = self.conformations[start: start +
                                     n_atoms].to(self.device).split(3, dim=1)
            pos = torch.stack(pos, dim=1)
            features = self.features_tensor[start: start +
                                            n_atoms].to(self.device)
            pos_mask = self.pos_masks[idx].to(self.device)
            chiral_tag = self.chiral_tags[start: start +
                                          n_atoms].to(self.device)
            ...
            for i, neighbor_slice in enumerate(neighbor_slices[:-1]):
                neighbors[neighbors_idx[i].item(
                )] = self.meta_dict['neighbors_list'][neighbor_slice: neighbor_slices[i + 1]].to(self.device)
            return torch_geometric.data.Data(
                x=features,
                pos=pos,
    ...
    def process(self):
        ...
        for smiles, sub_dic in tqdm(list(summary.items())):
            ...
            if os.path.isfile(pickle_path):
                ...
                for i, conformer in enumerate(conformers[:10]):
                    ...
                    # skip conf_mols with atoms with more than 4 neighbors for
                    # now
                    n_neighbors = [len(a.GetNeighbors())
                                   for a in conf_mol.GetAtoms()]
                    if np.max(n_neighbors) > 4:
                        continue

                    # filter for conformers that may have reacted
                    try:
                        conf_canonical_smi = Chem.MolToSmiles(
                            Chem.RemoveHs(conf_mol))
                    ...
                    confs_coords.append(
                        torch.tensor(
                            conf_mol.GetConformer().GetPositions(),
                            dtype=torch.float))
                    correct_mol = conf_mol
                    pos_mask[i] = 1

                if len(
                        confs_coords) == 0:  # skip mol for which we have no valid conformers
                    continue
                # if there are less than 10 confs_coords we add the first one a
                # few times
                if len(confs_coords) < 10:
                    confs_coords.extend(
                        [confs_coords[0]] * (10 - len(confs_coords)))
                ...
                # we need this additionally because uusing atom_slices for
                # slicing wont work since we only include the atoms with more
                # than 1 neighbor
                atoms_with_neighbors_slices.append(total_atoms_with_neighbors)

                all_atom_chiral_tags.append(
                    torch.tensor(chiral_tag, dtype=torch.float))
        ...
        data_dict = {'smiles': smiles_list,
                     ...
                     # very long tensor with neighbors for each atom indexed by
                     # neighbors_slices
                     'neighbors_slices': torch.tensor(neighbors_slices, dtype=torch.long),
                     # n_atoms times num_neighbors where num_neighbors changes
                     # for each atom. Slice out of this with
                     # atoms_with_neighbors_slices
