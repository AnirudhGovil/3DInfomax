...
class QM9DatasetGeomolConformers(Dataset):
    ...
    def data_by_type(self, idx, return_type, e_start, e_end, start, n_atoms):
        ...
        if return_type == 'mol_complete_graph':
            ...
            if idx in self.pairwise_distances:
                ...
                src, dst = self.get_pairwise(n_atoms)
                coords = self.coordinates[start: start +
                                          n_atoms].to(self.device)
                distances = torch.norm(
                    coords[src] - coords[dst], p=2, dim=-1).unsqueeze(-1).detach()
    ...
    def process(self):
        """ """
        print(
            'processing data from ({}) and saving it to ({})'.format(
                self.qm9_directory,
                os.path.join(
                    self.qm9_directory,
                    'processed_geomol_conformers')))
        ...
        geomol_structure_paths = os.listdir(
            os.path.join(
                self.qm9_directory,
                'geomol_predicted_qm9_structures'))
        geomol_structure_smiles = [os.path.splitext(
            path)[0] for path in geomol_structure_paths]
        data_qm9 = dict(
            np.load(
                os.path.join(
                    self.qm9_directory,
        ...
        for mol_idx, n_atoms in tqdm(enumerate(data_qm9['N'])):
           ...
           #    break
            ...
            mol = Chem.MolFromSmiles(
                molecules_df['smiles'][data_qm9['id'][mol_idx]])
