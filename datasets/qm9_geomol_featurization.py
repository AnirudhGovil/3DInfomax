...
class QM9GeomolFeaturization(Dataset):
        ...
        if self.prefetch_graphs and (
            ...
            for idx, n_atoms in tqdm(enumerate(self.meta_dict['n_atoms'])):
                ...
                self.mol_complete_graphs.append(dgl.heterograph(
                    {('atom', 'bond', 'atom'): (src, dst), ('atom', 'complete', 'atom'): (src, dst)}))
    ...
    def get_pairwise(self, n_atoms):
        ...
        if n_atoms in self.pairwise:
            ...
            dst = torch.cat([torch.cat([torch.arange(n_atoms)[:idx], torch.arange(
                n_atoms)[idx + 1:]]) for idx in range(n_atoms)])  # without self loops
    ...
    def get_mol_complete_graph(self, idx, e_start, e_end, n_atoms):
        ...
        if self.prefetch_graphs:
            ...
            edge_indices = self.edge_indices[:, e_start: e_end]
            src, dst = self.get_pairwise(n_atoms)
            g = dgl.heterograph({('atom', 'bond', 'atom'): (
                edge_indices[0], edge_indices[1]), ('atom', 'complete', 'atom'): (src, dst)})
        return g
    ...
    def data_by_type(self, idx, return_type, e_start, e_end, start, n_atoms):
        ...
        if return_type == 'dgl_graph':
            ...
            if self.dist_embedding:
                g.edata['d_rbf'] = self.dist_embedder(
                    g.edata['feat']).to(self.device)
            return g
        ...
        if return_type == 'san_graph':
            ...
            if self.e_features_tensor is not None:
                ...
                g.edges[edge_indices[0],
                        edge_indices[1]].data['feat'] = e_features
            ...
            return torch.ones_like(
                self.features_tensor[start: start + n_atoms], device=self.device)
            ...
            atom_features.extend(
                self.one_k_encoding(
                    atom.GetImplicitValence(), [
                        0, 1, 2, 3, 4, 5, 6]))
            atom_features.extend(self.one_k_encoding(
    ...
    def process(self):
        ...
        for mol_idx, n_atoms in tqdm(enumerate(data_qm9['N'])):
            ...
            # get all 19 attributes that should be predicted, so we drop the
            # first two entries (name and smiles)
            target = torch.tensor(
        ...
        data_dict = {
            ...
            'edge_features': torch.cat(
                all_edge_features,
                dim=0),
            'atomic_number_long': torch.tensor(
                data_qm9['Z'],
                dtype=torch.long)[
                :,
                None],
