...
class QM9Geom(Dataset):
    ...
    def __init__(
            ...
            normalize: bool = True,
            device='cuda:0',
            dist_embedding: bool = False,
            num_radial: int = 6,
            prefetch_graphs=True,
            transform=None,
            **kwargs):
        self.return_type_options = [
            'dgl_graph',
            'complete_graph',
            'dgl_graph3d',
            'complete_graph3d',
        ...
        if return_types is None:  # set default
            self.return_types: list = ['dgl_graph', 'targets']
        else:
            self.return_types: list = return_types
        for return_type in self.return_types:
            if return_type not in self.return_type_options:
                raise Exception(f'return_type not supported: {return_type}')

        if target_tasks is None or target_tasks == []:  # set default
            self.target_tasks = [
                'mu',
                'alpha',
                'homo',
        ...
        if features and 'constant_ones' in features or features3d and 'constant_ones' in features3d:
            data_dict['constant_ones'] = torch.ones_like(
                data_dict['atomic_number_long'], dtype=torch.float)
        if features and 'standard_normal_noise' in features or features3d and 'standard_normal_noise' in features3d:
            data_dict['standard_normal_noise'] = torch.normal(
                mean=torch.zeros_like(
                    data_dict['atomic_number_long'], dtype=torch.float), std=torch.ones_like(
                    data_dict['atomic_number_long'], dtype=torch.float))
        ...
        self.features_tensor = None if features == [] else torch.cat(
            [data_dict[k] for k in features], dim=-1)
        self.features3d_tensor = None if features3d == [] else torch.cat(
            [data_dict[k] for k in features3d], dim=-1)

        self.e_features_tensor = None if e_features == [] else torch.cat(
            [data_dict[k] for k in e_features], dim=-1)
        self.e_features3d_tensor = None if e_features3d == [] else torch.cat(
            [data_dict[k] for k in e_features3d], dim=-1)
        ...
        self.require_distances = any(
            return_type in self.return_types for return_type in [
                'dist_embedding',
                'pairwise_distances',
                'pairwise_indices',
                'complete_graph',
        ...
        if 'smiles' in self.return_types:
            self.smiles = pd.read_csv(
                os.path.join(
                    self.qm9_directory,
                    self.raw_qm9_file))['smiles']
        self.prefetch_graphs = prefetch_graphs
        if self.prefetch_graphs and any(
            return_type in self.return_types for return_type in [
                'dgl_graph',
                'dgl_graph3d',
                'se3Transformer_graph',
        ...
        if self.prefetch_graphs and (
            ...
            for idx in tqdm(range(len(self.meta_dict['edge_slices']) - 1)):
                ...
                self.complete_graphs.append(
                    dgl.graph((pairwise_indices[0], pairwise_indices[1])))
        if self.prefetch_graphs and (
        ...
        self.task_indices = torch.tensor(
            [list(self.unit_conversion.keys()).index(task) for task in self.target_tasks])
        # select targets in the order specified by the target_tasks argument

        self.targets = data_dict['targets'].index_select(
            dim=1, index=self.task_indices)  # [130831, n_tasks]
        ...
        # get a tensor that is 1000 for all targets that are energies and 1.0
        # for all other ones
    ...
    def data_by_type(
        ...
        if return_type == 'dgl_graph':
            ...
            g.ndata['f'] = self.features_tensor[start: start +
                                                n_atoms].to(self.device)
            g.ndata['x'] = self.coordinates[start: start +
                                            n_atoms].to(self.device)
            if self.e_features_tensor is not None:
                g.edata['w'] = self.e_features_tensor[e_start: e_end].to(
                    self.device)
            if self.pos_dir:
                g.ndata['pos_dir'] = self.pos_enc[start: start +
                                                  n_atoms].to(self.device)
            return g
        elif return_type == 'dgl_graph3d':
            g = self.get_graph(idx, e_start, e_end).to(self.device)
            g.ndata['f'] = self.features3d_tensor[start: start +
                                                  n_atoms].to(self.device)
            g.ndata['x'] = self.coordinates[start: start +
                                            n_atoms].to(self.device)
            if self.e_features3d_tensor is not None:
                g.edata['w'] = self.e_features3d_tensor[e_start: e_end].to(
                    self.device)
            if self.pos_dir:
                g.ndata['pos_dir'] = self.pos_enc[start: start +
                                                  n_atoms].to(self.device)
            ...
            g = self.get_complete_graph(
                idx, pairwise_start, n_atoms).to(
                self.device)
            g.ndata['f'] = self.features_tensor[start: start +
                                                n_atoms].to(self.device)
            g.ndata['x'] = self.coordinates[start: start +
                                            n_atoms].to(self.device)
            g.edata['d'] = self.dist_dict['pairwise_distances'][pairwise_start: pairwise_start +
                                                                n_atoms * (n_atoms - 1)].unsqueeze(-1).to(self.device)
            if self.e_features_tensor is not None:
                bond_features = self.e_features_tensor[e_start: e_end].to(
                    self.device)
                e_features = torch.zeros(
                    (n_atoms * n_atoms, bond_features.shape[1]), device=self.device)
                edge_indices = self.edge_indices[:, e_start: e_end]
                bond_indices = edge_indices[0] * n_atoms + edge_indices[1]
                e_features[bond_indices] = bond_features
                pairwise_indices = self.dist_dict['pairwise_indices'][:,
                                                                      pairwise_start: pairwise_start + n_atoms * (n_atoms - 1)]
                g.edata['w'] = e_features[pairwise_indices[0]
                                          * n_atoms + pairwise_indices[1]]
            if self.pos_dir:
                g.ndata['pos_dir'] = self.pos_enc[start: start +
                                                  n_atoms].to(self.device)
            return g
            ...
            g.edata['d'] = self.dist_dict['pairwise_distances'][pairwise_start: pairwise_start +
                                                                n_atoms * (n_atoms - 1)].unsqueeze(-1).to(self.device)
            if self.e_features3d_tensor is not None:
                bond_features = self.e_features3d_tensor[e_start: e_end].to(
                    self.device)
                e_features = torch.zeros(
                    (n_atoms * n_atoms, bond_features.shape[1]), device=self.device)
                edge_indices = self.edge_indices[:, e_start: e_end]
        ...
        if return_type == 'mol_complete_graph':
            ...
            g.ndata['x'] = self.coordinates[start: start +
                                            n_atoms].to(self.device)
            if self.e_features_tensor is not None:
                g.edges['bond'].data['w'] = self.e_features_tensor[e_start: e_end].to(
                    self.device)
            if self.pos_dir:
                g.ndata['pos_dir'] = self.pos_enc[start: start +
                                                  n_atoms].to(self.device)
        ...
        if return_type == 'san_graph':
            ...
            if self.e_features_tensor is not None:
                e_features = self.e_features_tensor[e_start: e_end].to(
                    self.device)
                g.edata['w'] = torch.zeros(
                    g.number_of_edges(),
                    e_features.shape[1],
                    dtype=torch.float32,
                    device=self.device)
                g.edata['real'] = torch.zeros(
                    g.number_of_edges(), dtype=torch.long, device=self.device)
                edge_indices = self.edge_indices[:, e_start: e_end].to(
                    self.device)
                g.edges[edge_indices[0],
                        edge_indices[1]].data['w'] = e_features
            ...
            if self.e_features_tensor is not None and return_type == 'se3Transformer_graph':
                g.edata['w'] = self.e_features_tensor[e_start: e_end].to(
                    self.device)
            elif self.e_features3d_tensor is not None and return_type == 'se3Transformer_graph3d':
                g.edata['w'] = self.e_features3d_tensor[e_start: e_end].to(
                    self.device)
            return g
        elif return_type == 'pairwise_indices':
            return self.dist_dict['pairwise_indices'][:,
                                                      pairwise_start: pairwise_start + n_atoms * (n_atoms - 1)]
        elif return_type == 'pairwise_distances':
            return self.dist_dict['pairwise_distances'][pairwise_start:
                                                        pairwise_start + n_atoms * (n_atoms - 1)].unsqueeze(-1)
        elif return_type == 'pairwise_distances_squared':
            return self.dist_dict['pairwise_distances'][pairwise_start: pairwise_start +
                                                        n_atoms * (n_atoms - 1)].unsqueeze(-1) ** 2
        elif return_type == 'dist_embedding':
            dist = self.dist_dict['pairwise_distances'][pairwise_start:
                                                        pairwise_start + n_atoms * (n_atoms - 1)]
    ...
    def process(self):
        ...
        print('processing data from ({}) and saving it to ({})'.format(
            self.qm9_directory, os.path.join(self.qm9_directory, 'processed')))

        # load qm9 data with spatial coordinates
        unpacker = msgpack.Unpacker(
            open('dataset/GEOM/qm9_crude.msgpack', "rb"))
        ...
        for pack in unpacker:
            for i, smiles in enumerate(pack.keys()):
                ...
                # add hydrogen bonds to molecule because they are not in the
                # smiles representation
                mol = Chem.AddHs(mol)

                for key, item in goli.features.get_mol_atomic_features_float(
                        mol, list(atom_float.keys())).items():
                ...
                for key, item in goli.features.get_mol_edge_features(
                        mol, list(e_features.keys())).items():
                    # repeat interleave for src dst and dst src edges (see
                    # above where we add the edges) and then reorder using perm
                    e_features[key].append(
                        torch.tensor(item).repeat_interleave(
                            2, dim=0)[perm])

                # get all 19 attributes that should be predicted, so we drop
                # the first two entries (name and smiles)
                target = torch.tensor(
                    molecules_df.iloc[data_qm9['id'][mol_idx]][2:], dtype=torch.float)
        ...
        targets = torch.stack(
            targets) * torch.tensor(list(self.unit_conversion.values()))[None, :]
    ...
    def process_distances(self):
        """ """
        print('processing distances from ({}) and saving it to ({})'.format(
            self.qm9_directory, os.path.join(self.qm9_directory, 'processed')))
        # load qm9 data with spatial coordinates
        data_qm9 = dict(
            np.load(
        ...
        for mol_idx, n_atoms in tqdm(enumerate(data_qm9['N'])):
            ...
            # without self loops
            dst = torch.cat([torch.cat([torch.arange(n_atoms)[:idx], torch.arange(
                n_atoms)[idx + 1:]]) for idx in range(n_atoms)])
            coordinates = torch.tensor(
                data_qm9['R'][total_atoms: total_atoms + n_atoms], dtype=torch.float)
