        ...
        for mol_idx, chembl_id in tqdm(enumerate(chembl_ids)):
            ...
            for conf_idx, sdf_name in enumerate(sdf_names):
                ...
                if conf_idx == 0:
                    ...
                    # get all 19 attributes that should be predicted, so we
                    # drop the first two entries (name and smiles)
                    targets['DFT:HOMO_ENERGY'].append(
                        float(mol.GetProp('DFT:HOMO_ENERGY')))
                    targets['DFT:TOTAL_ENERGY'].append(
                        float(mol.GetProp('DFT:TOTAL_ENERGY')))
                    targets['DFT:ATOMIC_ENERGY'].append(
                        float(mol.GetProp('DFT:ATOMIC_ENERGY')))
            ...
            if len(
                    conformers) < 3:  # if there are less than 10 conformers we add the first one a few times
                conformers.extend([conformers[0]] * (3 - len(conformers)))

            coordinates = torch.cat(
                [coordinates, torch.cat(conformers, dim=1)], dim=0)
