...
class DistancePredictor(nn.Module):
    ...
    def forward(
        ...
        # apply down projection to embeddings if we are not using a distance
        # net and projection_dim > 0
        if self.node_projection_net and not self.distance_net:
            graph.apply_nodes(self.node_projection)

        if self.transformer_layer:
            # put the embeddings h from the same graph in the batched graph
            # into pairs for the distance net to predict the pairwise distances
            feat = graph.ndata['feat']  # [n_atoms, hidden_dim]

            # [batch_size*(max_num_atoms), hidden_dim]
            expanded_mask = mask.view(-1).unsqueeze(1).expand(-1, hidden_dim)
            transformer_feat = torch.zeros_like(
                expanded_mask,
                device=mask.device,
                dtype=torch.float)  # [batch_size*(max_num_atoms), hidden_dim]
            transformer_feat[~expanded_mask] = feat.view(-1)
            transformer_feat = self.transformer_layer(
                transformer_feat.view(
                    batch_size,
                    max_num_atoms,
                    hidden_dim),
                src_key_padding_mask=mask)  # [batch_size, max_num_atoms, hidden_dim]
            feat = transformer_feat.view(
                batch_size * max_num_atoms,
                hidden_dim)[
                ~expanded_mask]  # [n_atoms*hidden_dim]
        ...
        if self.distance_net:
            ...
            distances = F.softplus(
                self.distance_net(src_dst_h) +
                self.distance_net(dst_src_h))
        else:
