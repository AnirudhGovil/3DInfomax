...
class OGBGNN(nn.Module):
    ...
    def __init__(
        ...
        if virtual_node:
            self.node_gnn = GNN_node_Virtualnode(
                num_layers,
                hidden_dim,
                JK=JK,
                dropout=dropout,
        ...
        if graph_pooling == "set2set":
            self.graph_pred_linear = nn.Linear(
                2 * self.hidden_dim, self.target_dim)
        else:
            self.graph_pred_linear = nn.Linear(
...
class GINConv(nn.Module):
    def __init__(self, hidden_dim, batch_norm_momentum=0.1):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(
                hidden_dim, hidden_dim), nn.BatchNorm1d(
                hidden_dim, momentum=batch_norm_momentum), nn.ReLU(), nn.Linear(
            ...
            self.mlp_virtualnode_list.append(
                nn.Sequential(
                    nn.Linear(
                        ...
                        hidden_dim, momentum=batch_norm_momentum), nn.ReLU(), nn.Linear(
                        hidden_dim, hidden_dim), nn.BatchNorm1d(
                        hidden_dim, momentum=batch_norm_momentum), nn.ReLU()))
        self.pool = SumPooling()
