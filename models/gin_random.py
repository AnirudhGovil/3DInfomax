...
class OGBGNNRandom(nn.Module):
    ...
    def __init__(
        ...
        if virtual_node:
            self.node_gnn = GNN_node_VirtualnodeRandom(
                num_layers,
                hidden_dim,
                JK=JK,
                dropout=dropout,
        ...
        if graph_pooling == "set2set":
            self.graph_pred_linear = nn.Linear(
                2 * self.hidden_dim, self.num_tasks)
        else:
    ...
    def forward(self, g):
        ...
        h_node, _ = self.node_gnn(
            rand_x=rand_x, rand_edge=rand_edge, dgl_graph=g, x=x, edge_attr=edge_attr)
...
class GNN_nodeRandom(nn.Module):
    ...
    def forward(self, rand_x, rand_edge, dgl_graph, x, edge_attr, **kwargs):
        ...
        dgl_graph.ndata['feat'] = torch.cat(
            [dgl_graph.ndata['feat'], rand_x], dim=-1)
        g = dgl_graph
        ...
        for layer in range(self.num_layers):
            ...
            h = self.convs[layer](
                g,
                h_list[layer],
                edge_attr=dgl_graph.edata['feat'].clone(),
                rand_edge=rand_edge)
        ...
        if self.pretrain_mode:
            node_representation = node_representation.view(
                n_atoms, -1, small_hidden_dim + self.random_vec_dim)
        else:
...
class GNN_node_VirtualnodeRandom(nn.Module):
    ...
    def __init__(
        ...
        self.atom_encoder = AtomEncoder(
            emb_dim=hidden_dim - self.random_vec_dim)
