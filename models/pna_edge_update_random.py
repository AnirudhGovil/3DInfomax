...
class PNARandomEdgeUpdate(nn.Module):
    def __init__(self,
        ...
        self.node_gnn = PNAGNNRandomEdgeUpdate(
    ...
    def forward(self, graph: dgl.DGLGraph):
        ...
        rand_x = rand_dist.sample([graph.ndata['feat'].size(
            0), self.random_vec_dim]).squeeze(-1).to(graph.device)
        rand_edge = rand_dist.sample([graph.edata['feat'].size(
            0), self.random_vec_dim]).squeeze(-1).to(graph.device)
...
class PNAGNNRandomEdgeUpdate(nn.Module):
    ...
    def forward(self, rand_x, rand_edge, dgl_graph: dgl.DGLGraph, **kwargs):
        ...
        dgl_graph.edata['feat'] = self.edge_init(
            torch.cat([dgl_graph.edata['feat'], rand_edge], dim=-1))
...
class PNALayerEdgeUpdate(nn.Module):
    def __init__(self,
        ...
        self.posttrans_1 = MLP(
            ...
            last_batch_norm=last_batch_norm,
            batch_norm_momentum=batch_norm_momentum)
        self.posttrans_2 = MLP(in_dim=(len(self.aggregators) * len(self.scalers)) * in_dim,
                               hidden_size=out_dim,
                               out_dim=out_dim,
                               layers=posttrans_layers,
                               mid_activation=activation,
