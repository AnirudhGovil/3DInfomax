...
class GRU(nn.Module):
    ...
    def forward(self, x, y):
        ...
        :return:    shape: (B, N, Dh)
        """
        assert (x.shape[-1] <= self.input_size and y.shape[-1]
                <= self.hidden_size)
        x = x.unsqueeze(0)
        y = y.unsqueeze(0)
        x = self.gru(x, y)[1]
...
class PNAOriginalRandom(nn.Module):
    def __init__(
        ...
        self.node_gnn = PNAGNNOriginalRandom(
            ...
            divide_input_first=divide_input_first,
            divide_input_last=divide_input_last,
            edge_feat=edge_feat,
            graph_norm=graph_norm,
            towers=towers)

        self.readout_aggregators = readout_aggregators
        self.output = MLPReadout(last_layer_dim *
                                 len(self.readout_aggregators), target_dim)
        ...
        self.layers = nn.ModuleList(
            [
                PNALayer(
                    ...
                    pretrans_layers=pretrans_layers,
                    posttrans_layers=posttrans_layers) for _ in range(
                    propagation_depth - 1)])
        self.layers.append(
            PNALayer(
                in_dim=hidden_dim,
                out_dim=last_layer_dim,
                dropout=dropout,
                graph_norm=graph_norm,
                mid_batch_norm=mid_batch_norm,
                last_batch_norm=last_batch_norm,
...
class PNATower(nn.Module):
    def __init__(
        ...
        self.pretrans = MLP(
            in_dim=2 * in_dim + (
                edge_hidden_dim if edge_features else 0) + (
                1 if use_3d else 0),
            hidden_size=in_dim,
            out_dim=in_dim,
    ...
    def pretrans_edges(self, edges):
        ...
        if self.use_3d:
            distances = torch.norm(
                (edges.src['x'] - edges.dst['x']), dim=-1)[:, None]
...
class PNALayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert ((not divide_input) or in_dim % towers ==
                0), "if divide_input is set the number of towers has to divide in_dim"
        assert (
            out_dim %
            towers == 0), "the number of towers has to divide the last_layer_dim"
    ...
    def forward(self, g, h, e, snorm_n):
        ...
        if self.divide_input:
            ...
            h_cat = torch.cat([tower(g, h, e, snorm_n)
                              for tower in self.towers], dim=1)
...
class PNAOriginalSimpleRandom(nn.Module):
    def __init__(
        ...
        self.node_gnn = PNAGNNSimpleRandom(
            hidden_dim=hidden_dim,
            last_layer_dim=last_layer_dim,
            last_batch_norm=last_batch_norm,
            mid_batch_norm=mid_batch_norm,
    ...
    def forward(self, g):
        ...
        rand_x = rand_dist.sample([g.ndata['feat'].size(
            0), self.random_vec_dim]).squeeze(-1).to(g.device)
        rand_edge = rand_dist.sample([g.edata['feat'].size(
            0), self.random_vec_dim]).squeeze(-1).to(g.device)
...
class PNAGNNSimpleRandom(nn.Module):
    def __init__(
        ...
        # 1 out dim since regression problem
        self.output = MLPReadout(last_layer_dim, 1)
    ...
    def forward(self, rand_x, rand_edge, dgl_graph: dgl.DGLGraph, **kwargs):
        ...
        dgl_graph.ndata['feat'] = self.node_init(
            torch.cat([dgl_graph.ndata['feat'], rand_x], dim=-1))
