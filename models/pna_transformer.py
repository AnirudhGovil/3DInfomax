...
class PNATransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.node_gnn = PNATransformerGNN(
    ...
    def forward(self, dgl_graph, h, pos_enc, mask):
        ...
        # [batch_size, max_num_atoms + 1, hidden_dim]
        h = self.node_gnn(dgl_graph, h, pos_enc, mask)
...
class PNATransformerGNN(nn.Module):
    ...
    def forward(self, graph, h, pos_enc, mask):
        ...
        # [batch_size, max_num_atoms * (hidden_dim - pos_enc_dim)]
        h = self.atom_encoder(h.view(-1, h.shape[-1]))
        # [batch_size, max_num_atoms, hidden_dim - pos_enc_dim]
        h = h.view(batch_size, max_num_atoms, -1)
        # [batch_size, max_num_atoms, num_eigvec, pos_enc_dim]
        pos_enc = self.pos_enc_mlp(pos_enc)
        # [batch_size, max_num_atoms, pos_enc_dim]
        pos_enc = pos_enc.nansum(dim=2)
        ...
        mask_include_vnode = torch.cat([torch.tensor(False, device=h.device).unsqueeze(
            0).expand(batch_size, -1), mask], dim=1)  # [batch_size, max_num_atoms + 1]

        mask_exclude_vnode = torch.cat([torch.tensor(True, device=h.device).unsqueeze(
            0).expand(batch_size, -1), mask], dim=1)  # [batch_size, max_num_atoms + 1]
...
class PNATransformerLayer(nn.Module):
    def __init__(self,
                 ...
                 posttrans_layers: int = 1,
                 pretrans_layers: int = 1,
                 combine_mlp_batchnorm_last=False,
                 simple=False,
                 **kwargs):
        ...
        self.combine_mlp = MLP(
            ...
            last_batch_norm=combine_mlp_batchnorm_last,
            out_dim=hidden_dim,
            layers=1,
            batch_norm_momentum=batch_norm_momentum)
    ...
    def forward(self, graph, h, mask_include_vnode, mask_exclude_vnode):
        ...
        # [batch_size, max_num_atoms + 1, hidden_dim]
        h_graph_padded = h.clone()
        # [batch_size*(max_num_atoms + 1), hidden_dim]
        h_graph_padded = h_graph_padded.view(-1, hidden_dim)
        # [batch_size*(max_num_atoms + 1), hidden_dim]
        expanded_mask = mask_exclude_vnode.view(
            -1).unsqueeze(1).expand(-1, hidden_dim)
        h_graph_padded[~expanded_mask] = h_graph.view(-1)

        # [batch_size, max_num_atoms + 1, hidden_dim]
        h_transformer = self.transformer_layer(
            h, src_key_padding_mask=mask_include_vnode)

        # [batch_size*(max_num_atoms + 1), 2*hidden_dim]
        h = torch.cat(
            [h_graph_padded, h_transformer.view(-1, hidden_dim)], dim=1)
        ...
        # [batch_size, max_num_atoms + 1, hidden_dim]
        h = h.view(batch_size, n_atoms_plus_one, hidden_dim)
        return h
