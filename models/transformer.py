...
class TransformerGNN(nn.Module):
    ...
    def forward(self, h, pos_enc, mask):
        ...
        # [batch_size, max_num_atoms + 1, hidden_dim]
        h = torch.cat([self.v_node[None, None, :].expand(
            batch_size, -1, -1), h], dim=1)

        mask = torch.cat([torch.tensor(False, device=h.device).unsqueeze(0).expand(
            batch_size, -1), mask], dim=1)  # [batch_size, max_num_atoms + 1]
