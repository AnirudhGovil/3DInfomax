...
def test_inference_3D_Model(args):
    '''
    Tests if the inference function is working or is empty. If this doesn't work, no structures will be generated from the smiles.
    '''
    assert inference(args) != -1

@pytest.mark.parametrize(
    "args",[get_arguments(2)]
)
def test_inference_2D_Model(args):
    '''
    Tests if the inference function is working or is empty. If this doesn't work, no structures will be generated from the smiles.
    '''
    assert inference(args) != -1
