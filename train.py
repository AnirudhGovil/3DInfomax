...
def get_trainer(args, model, data, device, metrics):
    ...
    if args.model3d_type:
        ...
        print('3D model trainable params: ', sum(p.numel()
              for p in model3d.parameters() if p.requires_grad))
...
def load_model(args, data, device):
    ...
    model = globals()[
        args.model_type](
        avg_d=data.avg_degree if hasattr(
            data,
            'avg_degree') else 1,
        device=device,
    ...
    if args.pretrain_checkpoint:
        ...
        # but only if they do not contain 'teacher' and remove 'student.' which
        # we need for loading from BYOLWrapper
        weights_key = 'model3d_state_dict' if args.transfer_3d else 'model_state_dict'
...
def train_geomol(args, device, metrics_dict):
    ...
    metric_name = [
        key for key in metrics_dict.keys() if 'ogbg-mol' +
        args.dataset.split('_')[0] == key.lower()][0]
...
def train_qm9_geomol_featurization(args, device, metrics_dict):
    ...
    test_idx = all_idx[len(model_idx): len(
        model_idx) + int(0.1 * len(all_data))]
    ...
    if 'qm9_properties' in args.metrics:
        metrics.update({task: QM9SingleTargetDenormalizedL1(
            dataset=all_data, task=task) for task in all_data.target_tasks})
...
def train_ogbg(args, device, metrics_dict):
    ...
    if args.force_random_split:
        ...
        split_idx["train"] = all_idx[len(split_idx["train"]):len(
            split_idx["train"]) + len(split_idx["valid"])]
        split_idx["train"] = all_idx[len(
            split_idx["train"]) + len(split_idx["valid"]):]
    collate_function = globals()[
        args.collate_function] if args.collate_params == {} else globals()[
        args.collate_function](
...
def train_geom(args, device, metrics_dict):
    ...
    test_idx = all_idx[len(model_idx): len(
        model_idx) + int(0.05 * len(all_data))]
    if args.dataset in ['file_loader_drugs', 'file_loader_qm9']:
        val_idx = all_idx[max(
            len(model_idx) + len(test_idx), len(all_data) - 1000):]
    else:
        val_idx = all_idx[len(model_idx) + len(test_idx):]
    train_idx = model_idx[:args.num_train]
    # for debugging purposes:
...
if __name__ == '__main__':
    ...
    if args.multithreaded_seeds != []:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            # list of tuples of dictionaries with the validation results first
            # and the test results second
        ...
        files = [
            open(
                os.path.join(
                    dir,
                    'multiple_seed_validation_statistics.txt'),
                'w') for dir in log_dirs]
        ...
        files = [
            open(
                os.path.join(
                    dir,
                    'multiple_seed_test_statistics.txt'),
                'w') for dir in log_dirs]
        print('Test results:')
