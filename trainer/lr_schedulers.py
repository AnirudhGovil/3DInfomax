...
class WarmUpWrapper:
    ...
    def __init__(
        ...
        self.wrapped_scheduler = globals(
        )[wrapped_scheduler](self.optim, **kwargs)
        self.start_lrs = []
        self.warmup_phase = 0
    ...
    def step(self, metrics=None):
        ...
        if self._step < self.total_warmup_steps:
            ...
            for i, p in enumerate(self.optim.param_groups):
                # update all parameters if there is only one entry specified
                # for the warmup steps otherwise only update the ones
                # corresponding to the current warmup phase
                if i <= warmup_phase or len(self.warmup_steps) == 1:
                    # interpolate between 0 and the final starting learning
                    # rate
                    interpolation_value = self._step - \
                        ([0] + list(self.warmup_steps.cumsum()))[warmup_phase] + 1
                    if self.warmup_steps[warmup_phase] == 0:
                        ...
                        if self.interpolation == 'linear':
                            ...
                            p['lr'] = self.start_lrs[i] * ((-np.cos((np.pi) * (
                                interpolation_value / self.warmup_steps[warmup_phase])) + 1) * 0.5)
                        else:
                            raise ValueError(
                                'interpolation not implemented:', self.interpolation)
    ...
    def state_dict(self):
        ...
        state_dict = {
            key: value for key,
            value in self.__dict__.items() if key != 'optim'}
