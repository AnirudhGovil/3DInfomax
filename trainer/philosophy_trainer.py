...
class PhilosophyTrainer(SelfSupervisedTrainer):
    # TODO: implement loading of checkpoints to continue training. The current
    # problem is that only one optimizer and one scheduler will be saved. We
    # need to overwrite save model and move the scheduler and optim loading
    # from statedict into the initialize_optim and initialize_scheduler
    # functions instead of having them in the constructor of Trainer
    ...
    def process_batch(self, batch, optim):
        peasant_loss, philosopher_loss, critic_loss, view2d, view3d = self.forward_pass(
            batch)
        if optim is not None:  # run backpropagation if an optimizer is provided
            peasant_loss.backward(
                inputs=list(
                    self.model.parameters()),
                retain_graph=True)
            self.optim.step()
            philosopher_loss.backward(
                inputs=list(
                    self.model3d.parameters()),
                retain_graph=True)
            self.optim3d.step()
    ...
    def predict(self,
                ...
                return_predictions: bool = False) -> Tuple[Dict,
                                                           Union[torch.Tensor,
                                                                 None],
                                                           Union[torch.Tensor,
                                                                 None]]:
        ...
        args = self.args
        total_metrics = {
            k: 0 for k in self.evaluate_metrics(
                torch.ones(
                    (2, 2), device=self.device), torch.ones(
                    (2, 2), device=self.device)).keys()}
        ...
        for i, batch in enumerate(data_loader):
            ...
            peasant_loss, philosopher_loss, critic_loss, predictions, targets = self.process_batch(
                batch, optim)

            with torch.no_grad():
                if self.optim_steps % args.log_iterations == args.log_iterations - \
                        1 and optim is not None:  # log every log_iterations during train
                    metrics_results = self.evaluate_metrics(
                        predictions, targets, batch)
                    metrics_results[type(
                        self.loss_func).__name__] = peasant_loss.item()
                    metrics_results[type(
                        self.critic_loss_func).__name__] = critic_loss.item()
                    ...
                    self.tensorboard_log(
                        ...
                        epoch=epoch)
                    print(
                        '[Epoch %d; Iter %5d/%5d] %s: peasant_loss: %.7f' %
                        (epoch, i + 1, len(data_loader), 'train', peasant_loss.item()))
                if optim is None:  # during validation or testing when we want to average metrics over all the data in that dataloader
                    metrics_results = self.evaluate_metrics(
                        predictions, targets, batch)
                    metrics_results[type(
                        self.loss_func).__name__] = peasant_loss.item()
        ...
        total_metrics = {k: v / len(data_loader)
                         for k, v in total_metrics.items()}
        epoch_predictions = torch.cat(
            epoch_predictions,
            dim=0) if return_predictions else None
    ...
    def initialize_optimizer(self, optim):
        normal_params = [
            v for k,
            v in self.model.named_parameters() if 'batch_norm' not in k]
        batch_norm_params = [
        ...
        normal_params3d = [
            v for k,
            v in self.model3d.named_parameters() if 'batch_norm' not in k]
        batch_norm_params3d = [
            v for k, v in self.model3d.named_parameters() if 'batch_norm' in k]
        self.optim3d = optim([{'params': batch_norm_params3d, 'weight_decay': 0},
                              {'params': normal_params3d}],
                             **self.args.optimizer_params)
        normal_params_critic = [
            v for k, v in self.critic.named_parameters() if 'batch_norm' not in k]
        batch_norm_params_critic = [
            v for k, v in self.critic.named_parameters() if 'batch_norm' in k]
    ...
    def initialize_scheduler(self):
        if self.args.lr_scheduler:  # Needs "from torch.optim.lr_scheduler import *" to work
            ...
            self.lr_scheduler3d = globals()[self.args.lr_scheduler](
                self.optim3d, **self.args.lr_scheduler_params)
            self.lr_scheduler_critic = globals()[self.args.lr_scheduler](
                self.optim_critic, **self.args.lr_scheduler_params)
        else:
