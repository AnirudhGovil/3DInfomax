...
class SelfSupervisedAETrainer(SelfSupervisedTrainer):
    ...
    def forward_pass(self, batch):
        ...
        loss_contrastive, loss_reconstruction = self.loss_func(
            view2d, view3d, distance_preds, distances)
        return loss_contrastive, loss_reconstruction, view2d, view3d

    def process_batch(self, batch, optim):
        loss_contrastive, loss_reconstruction, predictions, targets = self.forward_pass(
            batch)
    ...
    def predict(self,
        ...
        for i, batch in enumerate(data_loader):
            ...
            loss_contrastive, loss_reconstruction, predictions, targets = self.process_batch(
                batch, optim)
            with torch.no_grad():
                if self.optim_steps % self.args.log_iterations == 0 and optim is not None:
                    metrics = self.evaluate_metrics(predictions, targets)
                    metrics[type(self.loss_func).__name__] = loss_contrastive.item(
                ...
                if optim is None and self.val_per_batch:
                    ...
                    metrics_results[type(self.loss_func).__name__] = loss_contrastive.item(
                    ) + loss_reconstruction.item()
