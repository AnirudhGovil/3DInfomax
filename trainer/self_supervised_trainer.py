...
class SelfSupervisedTrainer(Trainer):
    ...
    def forward_pass(self, batch):
        ...
        view2d = self.model(*info2d, *snorm_n)
        view3d = self.model3d(*info3d)
        loss = self.loss_func(
            view2d,
            view3d,
            nodes_per_graph=info2d[0].batch_num_nodes() if isinstance(
                info2d[0],
                dgl.DGLGraph) else None)
        return loss, view2d, view3d

    def evaluate_metrics(self, z2d, z3d, batch=None,
        ...
        if 'Local' in type(self.loss_func).__name__ and batch is not None:
            ...
            for graph_idx in range(1, len(node_indices)):
                pos_mask[node_indices[graph_idx - 1]
                    : node_indices[graph_idx], graph_idx] = 1.
    ...
    def run_per_epoch_evaluations(self, data_loader):
        ...
        for batch in data_loader:
            ...
            if len(representations) * \
                    len(view2d) >= self.args.linear_probing_samples:
                break
    ...
    def initialize_optimizer(self, optim):
        normal_params = [
            ...
            v in chain(
                ...
                self.model3d.named_parameters()) if 'batch_norm' not in k]
        batch_norm_params = [
            v for k,
            v in chain(
                self.model.named_parameters(),
                self.model3d.named_parameters()) if 'batch_norm' in k]
